...
class ProductList extends JFrame implements ActionListener
{
	...
	ProductList()
	{
		...
		try {
			...
			if (comfirm.equals(null)) {
				...
				enableVisibility();
				ProductList.this.setVisible(true);
				return;
			}
...
public static void main(String args []){
	ProductList sa1=new ProductList();
...
private String checkDatabase() throws SQLException{
   ...
   ResultSet rs = databaseConnection.statement.executeQuery("select * from product_addition");
	while(rs.next())
	{
		 name = rs.getString("idnum").trim();
...
private void compute(){
	try {
		databaseConnection.open();
		//checkDatabase();
		databaseConnection.resultSet = databaseConnection.statement.executeQuery("select * from product_addition ");
		int row=0;
		int i=0;
		...
		DefaultTableModel model=new DefaultTableModel(new String[]{"id","Date","time","Name","Category"},row);
		...
		while(databaseConnection.resultSet.next())
		{
			...
			model.setValueAt(databaseConnection.resultSet.getString(2),i,0);
			model.setValueAt(databaseConnection.resultSet.getString(3).trim(),i,1);
			model.setValueAt(databaseConnection.resultSet.getString(4).trim(),i,2);
			model.setValueAt(databaseConnection.resultSet.getString(5).trim(),i,3);
			model.setValueAt(databaseConnection.resultSet.getString(9).trim(),i,4);
